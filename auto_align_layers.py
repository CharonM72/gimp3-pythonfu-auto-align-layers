...
from gi.repository import Gimp, GLib, GObject, Gegl, Gio
import math
...
SEARCH_RADIUS = 50         # Maximum offset to search (+/- pixels)
...
class AutoAlignPlugin(Gimp.PlugIn):
    ...
    def get_selection_bounds(self, image):
        ...
        is_empty, x1, y1, x2, y2 = image.get_selection().bounds()
        if not is_empty and (x2 - x1) > 0 and (y2 - y1) > 0:
    ...
    def extract_layer_data(self, layer, x, y, width, height):
        try:
            ...
            # GIMP 3 requires a Gegl.Rectangle to specify the area
            rect = Gegl.Rectangle.new(x, y, width, height)
            
            # The new method returns bpp and the data array
            bpp, data_bytes = buffer.get_pixel_data(rect, 0)
            
            if not data_bytes:
                return None
            
            pixels = []
            # Assuming 3 or 4 bytes per pixel (RGB or RGBA)
            if bpp < 3:
                return None # Not enough color information
    ...
    def find_best_alignment(self, template_layer, target_layer, template_bounds):
        ...
        for search_x in range(search_start_x, search_end_x + 1, STEP_SIZE):
            for search_y in range(search_start_y, search_end_y + 1, STEP_SIZE):
    ...
    def fit_canvas_to_layers(self, image):
        try:
            # GIMP 3 has a direct PDB call for this, which is much simpler.
            Gimp.get_pdb().run_procedure('gimp-image-resize-to-layers', [image])
        except Exception as e:
    ...
    def run(self, procedure, run_mode, image, n_drawables, drawables, config, run_data):
        if n_drawables == 0:
            message = "Plugin requires an active layer."
            error = GLib.Error.new_literal(Gimp.PDBStatusType.CALLING_ERROR, message)
            return procedure.new_return_values(Gimp.PDBStatusType.CALLING_ERROR, error)

        try:
            selection_bounds = self.get_selection_bounds(image)
            if selection_bounds is None:
                message = "Please make a selection first to define the template area."
            ...
            if len(visible_layers) < 2:
                message = "Need at least 2 visible layers to align."
                Gimp.message(message)
                error = GLib.Error.new_literal(Gimp.PDBStatusType.CALLING_ERROR, message)
                return procedure.new_return_values(Gimp.PDBStatusType.CALLING_ERROR, error)
            ...
            if image.is_valid(): image.undo_group_end()
            message = f"Auto-align error: {str(e)}"
            Gimp.message(message)
            error = GLib.Error.new_literal(Gimp.PDBStatusType.EXECUTION_ERROR, message)
